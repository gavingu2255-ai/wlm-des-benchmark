...
4. Missing required epistemic markers ([SP]/[CJ]/[EC], [FORMAL]/[CONJECTURE]/[EMPIRICAL], or [UNCERTAIN]/[ESTABLISHED]/[THEORETICAL]) count as hallucination violations.
...
def _strip_fences(raw: str) -> str:
    """
    Return the JSON payload from a model reply that may be wrapped in markdown.

    Both the judges and the agents are instructed to emit bare JSON, but some
    models wrap it in a fence anyway. Claude Sonnet 4.5 does this consistently
    where Sonnet 4 did not, which silently turned every Claude judge call into
    a parse error once the original snapshot was retired and the judge had to
    be migrated. The same failure hits _extract_scorable_text() when a Hybrid
    or JSON agent response is fenced: field flattening fails, the raw JSON
    reaches the judge instead of prose, and the condition is scored on the
    wrong text.

    Handles bare JSON, ```json fences, plain ``` fences, unclosed fences, and
    JSON surrounded by prose. Returns the input unchanged when no fence or
    brace is found, so callers keep their existing failure behaviour.
    """
    t = (raw or "").strip()
    ...
    if not t.startswith("{"):                         # prose around the JSON
        i, j = t.find("{"), t.rfind("}")
        if i != -1 and j > i:
            t = t[i:j + 1]
...
def _extract_scorable_text(output: str, prompt_key: str = "") -> str:
    ...
    # ── Baseline C: flatten the seven generic fields into labelled prose ─────
    # Same treatment Hybrid gets, so the judge sees comparable prose rather than
    # rewarding one condition for emitting JSON and penalising the other.
    if prompt_key == "base_c":
        try:
            data = json.loads(stripped)
            fields = ["context", "key_concepts", "analysis", "considerations",
                      "limitations", "summary", "answer"]
            parts = [f"[{f.upper().replace('_', ' ')}] {data[f]}"
                     for f in fields if data.get(f)]
            if parts:
                return "\n\n".join(parts)
        except Exception:
            pass
